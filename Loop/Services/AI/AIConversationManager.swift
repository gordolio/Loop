...
//  Loop
//
//  Created for AI-assisted carb entry chat feature.
...
final class AIConversationManager: ObservableObject {
    ...
    func initialize(
        ...
        self.currentItems = response.foodItems
        self.selectedItemIds = Set(response.foodItems.map { $0.id })
        self.initialReasoning = response.reasoning
        ...
        os_log("Initialized conversation with %d items, total %.1fg carbs",
               log: log, type: .info, currentItems.count, totalCarbs)
    }
    ...
    @MainActor
    func updateItemDescription(itemId: UUID, newDescription: String) async {
        ...
        os_log("Updating item '%{public}@' to '%{public}@'",
               log: log, type: .info, oldName, newDescription)
        ...
        do {
            ...
            os_log("Failed to update item: %{public}@",
    ...
    func sendMessage(_ text: String) async {
        ...
        let allItemIds = Set(currentItems.map { $0.id })
        ...
        do {
            ...
            let newItemIds = Set(response.foodItems.map { $0.id })
            selectedItemIds = selectedItemIds.intersection(newItemIds).union(
                newItemIds.subtracting(Set(currentItems.map { $0.id }))
            )
            ...
            os_log("Conversation turn complete: %d items updated",
                   log: log, type: .info, response.updatedItemIds.count)

        } catch {
            os_log("Conversation turn failed: %{public}@",
                   log: log, type: .error, error.localizedDescription)
            errorMessage = error.localizedDescription
